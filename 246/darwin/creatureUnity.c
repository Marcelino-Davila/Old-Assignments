...
#define MAKE_CREATURE newCreature(1,createSpecies("mongrol"),newWorld("Krypton",20,50),createPoint(5,10),NORTH)
...
void setWorld_(void) {
  creature c = MAKE_CREATURE;
  setWorld(c, newWorld("Vulcan",20,50));
  char *name = getName(getWorld(c));
...
void getSpecies_(void) {
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE("mongrol",getFileName(s),"getSpecies #1 failed");
}

void setSpecies_(void) {
  creature c = MAKE_CREATURE;
  setSpecies(c,createSpecies("test"));
  char *name = getFileName(getSpecies(c));
  TEST_ASSERT_EQUAL_STRING_MESSAGE("test",name,"setSpecies #1 failed");  
}

void getPC_(void) {
  creature c = MAKE_CREATURE;
  TEST_ASSERT_EQUAL_INT_MESSAGE(0,getPC(c),"getPC #1 failed");
}
...
void toCreatureString_(void) {
  creature c = MAKE_CREATURE;
  TEST_ASSERT_EQUAL_STRING_MESSAGE("(mongrol,#1,(row:5,col:10),north)",toCreatureString(c),"toCreatureString #1 failed");
}
